...
/* taken from the PHP iCalendar project, version 2.24 */
/* minor modifications by me -ARN */
if (!defined('BASE')) define('BASE', './');
// -ARN: this include is not needed 
...
// -ARN: taken from phpicalendar error.php
...
foreach ($cal_filelist as $cal_key=>$filename) {
	...
	if ($parse_file) {	
		...
		//print($nextline . "<br \>");
		if (trim($nextline) != 'BEGIN:VCALENDAR' && trim($nextline) != 'GIN:VCALENDAR') exit(error($lang['l_error_invalidcal'], $filename));
